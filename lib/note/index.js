...
async function getAllNotes (callback) {
  ...
  try {
    const noteList = notes
      .filter(n => {
        n.tempTags = Note.parseNoteInfo(n.content).tags
        return !n.tempTags.some(t => t.toLowerCase() === 'uncompany')
      })
      .map(note => {
        return ({
          id: Note.encodeNoteId(note.id),
          ...
          lastchangeAt: note.lastchangeAt,
          shortId: note.shortid,
          owner: note.owner ? User.getProfile(note.owner).name : null,
          lastchangeuser: note.lastchangeuser ? User.getProfile(note.lastchangeuser).name : null
        })
      })
    if (config.debug) {
      logger.info('Parse noteList success')
    }
    return callback(null, noteList)
  } catch (err) {
    logger.error('Parse noteList failed')
    return callback(err, null)
  }
}
...
function listAllNotes (req, res) {
  if (req.isAuthenticated()) {
    getAllNotes((err, noteList) => {
      if (err) return errorInternalError(req, res)
      if (!noteList) return errorNotFound(req, res)
      res.send({
        notes: noteList
      })
    })
